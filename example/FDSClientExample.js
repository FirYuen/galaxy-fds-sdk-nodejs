...
(async ()=>{
    ...
    // Check Bucket
    var exists = await client.doesBucketExist(bucketName);
    ...
    await client.putObject(bucketName, objectName, objectContent);

    // Get the object content
    var obj = await client.getObject(bucketName, objectName);
    console.log('-- 4. Get Object content:');
    obj.stream.pipe(process.stdout);
    await FDSClient.streamToPromise(obj.stream);

    // Download the object file
    console.log('-- 5. Download object file');
    var dataFile = '/tmp/fds_file';
    await client.downloadObject(bucketName, objectName, dataFile);
    exists = fs.existsSync(dataFile);
    console.log('-- 5. Download object file exists: ' + true);
    var dataFile2 = '/tmp/fds_file2';
    await client.downloadObjectWithUri('fds://' + bucketName + '/' + objectName, dataFile2);
    exists = fs.existsSync(dataFile);
    ...
    await FDSClient.streamToPromise(req);
    ...
    await client.putObject(bucketName, objectName, 'shared content\n');
    ...
    await client.setObjectAcl(bucketName, objectName, objectAcl);

    // Read the shared object by other client
    console.log('-- 13. Get object by other client:');
    obj = await other_client.getObject(bucketName, objectName);
    obj.stream.pipe(process.stdout);
    await FDSClient.streamToPromise(obj.stream);
    ...
    await client.setBucketAcl(bucketName, bucketAcl);

    await other_client.deleteObject(bucketName, objectName);

    // Post an object by others
    var result = await other_client.postObject(bucketName, 'post object by other client\n');
    console.log('-- 15. Post an object and returned object_name is:');
    console.log(result.objectName);
    await other_client.deleteObject(bucketName, result.objectName);
    ...
    await other_client.putObject(bucketName, objectName, rs);
    exists = await other_client.doesObjectExsits(bucketName, objectName);
    console.log(format('-- 16. Stream object file exists: %s', exists));

    // --------------
    // List objects
    var result = await client.listObjects(bucketName);
